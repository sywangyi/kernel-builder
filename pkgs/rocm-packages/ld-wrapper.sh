#! @shell@
# This file is vendored from nixpkgs. This is to work around a whacky
# issue with lld. The compiler ends up calling ld.lld with `-flavor gnu`.
# However, this option is only accepted by `lld`, which should not be
# called directly. So we adjust the ld warpper to filter any `-flavor`
# options that are passed. There must be a better way to do this, but
# this works for now.
